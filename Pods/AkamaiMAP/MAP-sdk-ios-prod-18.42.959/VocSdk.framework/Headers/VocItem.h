...
/*! @brief This property can be used to arrange order of downloads. Items with higher value will get downloaded before items with lower value. By default this property is 0. It works for both automatic background downloads and manual downloads through [VocService downloadItems:] API. When [VocService downloadItems:] API is used this property is set to 1 unless it already has a higher value. So if manually invoking download and want to control the order set this property before calling [VocService downloadItems:] API.
 */
@property (readwrite,assign,nonatomic)			int16_t				userPriority;

/*!
 @brief Deprecated - use userPriority instead.
 */
@property (readwrite,assign,nonatomic)			BOOL				prioritizedDownload VOCSDK_DEPRECATED;
...
/*! @brief Item is scheduled to download later. The metadata will be fetched on serverAvailableDate. 
	The client can remove the item from download later queue by setting downloadLater = NO.
	It is an invalid operation to set downloadLater to YES from NO.
 */
@property (readwrite,assign,nonatomic)			BOOL				downloadLater;

/*! @brief The date on which the item needs to be synced. This is associated with item marked for download later.
 */
@property (readonly,nullable,strong,nonatomic)	NSDate				*serverAvailableDate;

/*!
   @brief Points to the error why the item failed to download. This property holds the last download error. Once reported, SDK does not reset the error until the item gets into VOCItemCached, VOCItemPartiallyCached, VOCItemDeleted or VOCItemDiscovered state.

   @discussion
   The property will be non nil for VocItemFailed state.
   Once reported, the property value will not be cleared until the item move into states such as VOCItemCached, VOCItemPartiallyCached, VOCItemDeleted or VOCItemDiscovered.
   If download fails for any reason, this property will hold the error object with the reason for the failure.
   If download stops due to issues that user or server has to take action, it will still report it.
   If download stops for cases such as item metadata got updated due to server action or max concurrent downloads exceeded, it will not report it as an error.
 */
@property (readonly,nullable,nonatomic)			NSError						*downloadError;


/*
	@brief Shows how the item was imported to SDK. Either user(application) has requested for this content (VocDownloadTypeManual) or it was imported automaicaly by SDK.
 */
@property (readonly,assign,nonatomic)			VocDownloadType				downloadType;
...
/*!
   @brief Delete content in the file system but not the metadata. The item will be moved to VOCItemDiscovered state and downloadBehavior will be set to VOCItemDownloadNone.
...
/*!
 @method
 @brief Provides AVURLAsset for playback. If the video is not cached the asset will be be pointintg to online url, otherwise
 	it will point to local data and can be played offline. If for some reason asset cannot be created asset will be nil and error
 	will be set.
 */
- (void)getAVAssetWithCompletion:(nonnull void (^)(AVURLAsset* __nullable asset, NSError * __nullable error))completion;
...
/*!
   @brief DEPRECATED - use -getAVAssetWithCompletion instead. For HLS content, SDK sets up a localhost server. This property points to the relative path that needs to be added to localhost server url for accessing the master m3u8 file.

 */
@property (readonly,nullable,strong,nonatomic)	NSString 	*hlsServerRelativePath VOCSDK_DEPRECATED;
